...
func (exec *InsertExec) run(ctx context.Context, output chan<- *Result, outer, inner <-chan *Result) error {
	...
	for {
		...
		case res, ok := <-outer:
			...
			err := command.InsertTableEntry(exec.tbl, res.Slot.Values)
			if err != nil {
				return err
			}
			exec.num++
...
func (exec *InsertExec) message(num int) *Result {
